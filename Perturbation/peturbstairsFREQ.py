...
filename = 'FREQ' + expinfo['observer'] + expinfo['dateStr']

    #setup staircase
stairs=data.StairHandler(startVal=2, stepType='lin', maxVal=5, minVal=0, nUp=1, nDown=3, nReversals=4, nTrials=20, stepSizes=[0.4,0.2,0.2,0.1])
...
#        np.roll(xrange[n], temp[n])
    ...
    return intensity
#
myWin = visual.Window(size = (800,600), units = 'deg', monitor = 'testMonitor', fullscr=True)
...
for freq in stairs:
    ...
    grating1 = makeSineGrating(128, cycles=10.0, amplitude=2, frequency=freq)   #sets the value from staircase
