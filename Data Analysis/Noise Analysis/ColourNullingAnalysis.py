...
#need to make all x vals positive
combinedInten=pylab.array(combinedInten)+100.0
#fit curve
fit = data.FitWeibull(combinedInten, combinedResp, guess=[100.0, 0.0], expectedMin = 0.0)
...
#plot points
#pylab.plot(combinedInten-100.0, combinedResp, 'ko')
#plot points
...
for pointN in range(len(combinedInten)):
    ...
    pylab.plot([combinedInten[pointN]-100], [combinedResp[pointN]], 'ok', markerfacecolor='w', label = '_nolegend_',markeredgewidth=1.0, markersize=thisMarkerSize)     
pylab.ylim([0,1])
