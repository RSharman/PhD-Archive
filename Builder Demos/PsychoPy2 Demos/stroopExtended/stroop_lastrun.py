...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.65.01), November 03, 2011, at 15:07
...
#Initialise components for routine:instructPractice
...
#Start of routine instructPractice
t=0; instructPracticeClock.reset()
frameN=-1

#update component parameters for each repeat
ready1 = event.BuilderKeyResponse() #create an object of type KeyResponse
ready1.status=NOT_STARTED
#keep track of which have finished
instructPracticeComponents=[]#to keep track of which have finished
instructPracticeComponents.append(instr1)
instructPracticeComponents.append(ready1)
...
while continueRoutine:
    ...
    if t>=0.0 and ready1.status==NOT_STARTED:
        #keep track of start time/frame for later
        ready1.tStart=t#underestimates by a little under one frame
        ready1.frameNStart=frameN#exact frame index
        ready1.status=STARTED
        #keyboard checking is just starting
        event.clearEvents()
    if ready1.status==STARTED:#only update if being drawn
...
#end of routine instructPractice
for thisComponent in instructPracticeComponents:
    if hasattr(thisComponent,"setAutoDraw"): thisComponent.setAutoDraw(False)

#set up handler to look after randomisation of conditions etc
practice=data.TrialHandler(nReps=1.0, method='random', 
    extraInfo=expInfo, originPath='C:\\Documents and Settings\\lpxrs\\My Documents\\Code\\Experiments\\Builder Demos\\PsychoPy2 Demos\\stroopExtended\\stroop.psyexp',
...
for thisPractice in practice:
    ...
    #abbrieviate parameter names if possible (e.g. rgb=thisPractice.rgb)
    if thisPractice!=None:
        for paramName in thisPractice.keys():
            exec(paramName+'=thisPractice.'+paramName)
...
practice.saveAsPickle(filename+'practice')
practice.saveAsExcel(filename+'.xlsx', sheetName='practice',
    stimOut=practice.trialList[0].keys(),
