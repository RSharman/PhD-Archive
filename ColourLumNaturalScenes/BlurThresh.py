...
#Check which participant is testing and use the appropriate thresholds.
if info['participant']=='RJS':
    AchromThreshLeaf = 0.029
    AchromThreshPansy = 0.023
    AchromThreshPelican = 0.034
    AchromThreshPumpkin = 0.057
    IsolumThreshLeaf = 0.074
    IsolumThreshPansy = 0.074
    IsolumThreshPelican = 0.279
    IsolumThreshPumpkin = 0.036

if info['participant']=='SH':
    AchromThreshLeaf = 0.036
    AchromThreshPansy = 0.02
    AchromThreshPelican = 0.037
    AchromThreshPumpkin = 0.037
    IsolumThreshLeaf = 0.076
    IsolumThreshPansy = 0.071
    IsolumThreshPelican = 0.176
    IsolumThreshPumpkin = 0.034

if info['participant']=='AP':
    AchromThreshLeaf = 0.035
    AchromThreshPansy = 0.024
    AchromThreshPelican = 0.034
    AchromThreshPumpkin = 0.042
    IsolumThreshLeaf = 0.087
    IsolumThreshPansy = 0.099
    IsolumThreshPelican = 0.390
    IsolumThreshPumpkin = 0.023
    
if info['participant']=='KB':
    AchromThreshLeaf = 0.036
    AchromThreshPansy = 0.025
    AchromThreshPelican = 0.041
    AchromThreshPumpkin = 0.048
    IsolumThreshLeaf = 0.132
    IsolumThreshPansy = 0.069
    IsolumThreshPelican = 0.156
    IsolumThreshPumpkin = 0.034
    
if info['participant']=='DJH':
    AchromThreshLeaf = 0.026
    AchromThreshPansy = 0.023
    AchromThreshPelican = 0.026
    AchromThreshPumpkin = 0.044
    IsolumThreshLeaf = 0.079
    IsolumThreshPansy = 0.088
    IsolumThreshPelican = 0.088
    IsolumThreshPumpkin = 0.034
...
#        dklPicture = np.array(dklPicture)
        if pictureName == 'Leaf512.jpg':
            lum = copy.copy(dklPicture[:,:,0])*info['Luminance']*AchromThreshLeaf*info['threshMultiple']
            lm = copy.copy(dklPicture[:,:,1])*info['Chromaticity']*IsolumThreshLeaf*info['threshMultiple']
            s = copy.copy(dklPicture[:,:,2])*info['Chromaticity']*IsolumThreshLeaf*info['threshMultiple']

        if pictureName == 'Pansy512.jpg':
            lum = copy.copy(dklPicture[:,:,0])*info['Luminance']*AchromThreshPansy*info['threshMultiple']
            lm = copy.copy(dklPicture[:,:,1])*info['Chromaticity']*IsolumThreshPansy*info['threshMultiple']
            s = copy.copy(dklPicture[:,:,2])*info['Chromaticity']*IsolumThreshPansy*info['threshMultiple']
            
        if pictureName == 'Pelican512.jpg':
            lum = copy.copy(dklPicture[:,:,0])*info['Luminance']*AchromThreshPelican*info['threshMultiple']
            lm = copy.copy(dklPicture[:,:,1])*info['Chromaticity']*IsolumThreshPelican*info['threshMultiple']
            s = copy.copy(dklPicture[:,:,2])*info['Chromaticity']*IsolumThreshPelican*info['threshMultiple']
            
        if pictureName == 'Pumpkin512.jpg':
            lum = copy.copy(dklPicture[:,:,0])*info['Luminance']*AchromThreshPumpkin*info['threshMultiple']
            lm = copy.copy(dklPicture[:,:,1])*info['Chromaticity']*IsolumThreshPumpkin*info['threshMultiple']
            s = copy.copy(dklPicture[:,:,2])*info['Chromaticity']*IsolumThreshPumpkin*info['threshMultiple']
...
if not os.path.isdir('BlurThresh_%s' %info['participant']):
    os.mkdir('BlurThresh_%s' %info['participant'])
#
...
for thisStair in stairs:
    for p, d in thisStair.extraInfo.iteritems():
        Picture = p
        fName = 'BlurThresh_%s//BlurThresh_%s_%s_%s_%s_%s' %(info['participant'], Picture, dispInfo, blurInfo, info['dateStr'], info['participant'])
