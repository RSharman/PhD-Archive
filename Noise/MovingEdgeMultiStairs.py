...
#Conds = ['LM', 'S', 'Lum', 'LMLum', 'SLum', 'LMS']
Conds = ['LMLum', 'SLum', 'LMS']
shuffle(Conds)
print Conds
...
#Set Up 
    ...
    #Save stairs
    if not os.path.isdir('MultiSynthMov_%s' %info['participant']):
        os.mkdir('MultiSynthMov_%s' %info['participant'])
    fName = 'MultiSynthMov_%s//SynthMov_Gap%.2f_NContrLM%.2f_NContrS%.2f_NContrS%.2f_EContrLM%.2f_EContrS%.2f_EContrS%.2f_%s_%s_%s' %(info['participant'], info['Gap'], info['Noise Contrast LM'], \
