...
colorTex = flatAzim(512, 'LM', cycles=1.0, elevation=0.0)/2.0

#Create stimuli (much bigger for debugging)
if DEBUG==True:
    bwGabor1 = visual.PatchStim(myWin, tex='sin', mask='gauss', units = 'deg', size = 10.0, sf = 0.2,contrast=0.1)
    colorGabor1 = visual.PatchStim(myWin, tex=colorTex, mask = 'gauss', units = 'deg', size = 10.0, sf = 0.2)
    bwGabor2 = visual.PatchStim(myWin, tex='sin', mask='gauss', units = 'deg', size = 10.0, sf = 0.2,contrast=0.1)
    colorGabor2 = visual.PatchStim(myWin, tex=colorTex, mask = 'gauss', units = 'deg', size = 10.0, sf = 0.2)
if DEBUG==False:
    bwGabor1 = visual.PatchStim(myWin, tex='sin', mask='gauss', units = 'deg', size = 2.0, sf = 2.0,contrast=0.1)
    colorGabor1 = visual.PatchStim(myWin, tex=colorTex, mask = 'gauss', units = 'deg', size = 2.0, sf = 2.0)
    bwGabor2 = visual.PatchStim(myWin, tex='sin', mask='gauss', units = 'deg', size = 2.0, sf = 2.0,contrast=0.1)
    colorGabor2 = visual.PatchStim(myWin, tex=colorTex, mask = 'gauss', units = 'deg', size = 2.0, sf = 2.0)
...
for trialN in range(info['nTrials']):
    ...
    #Change the direction of apparent motion
        if direction=='left':
            bwGabor1.setPhase(0.0)
            colorGabor1.setPhase(0.25)
            bwGabor2.setPhase(0.5)
            colorGabor2.setPhase(0.75)
        if direction=='right':
            bwGabor1.setPhase(0.0)
            colorGabor1.setPhase(0.75)
            bwGabor2.setPhase(0.5)
            colorGabor2.setPhase(0.25)
        #update stimulus colors
...
for thisStair in stairs:
    fName = 'ColorNulling_%s//ColorNulling_%s_%s_StartVal%.2f_%s' %(info['participant'], thisStair.extraInfo['participant'], thisStair.extraInfo['condition'], thisStair.extraInfo['startVal'], info['dateStr'])
