...
info['nTrials'] = 1
info['nReversals'] = 1
info['stepSizes'] = [1.0,1.0,0.5,0.5,0.25,0.25,0.125,0.125,0.06,0.06]#[0.1, 0.1, 0.05, 0.05, 0.025, 0.025, 0.0125]
info['minVal'] = 0
info['maxVal'] = 10
info['startVal'] = 5
info['nUp'] = 1
...
#    gapDeg = info['Gap']/2
    ...
    lm= colorFunctions.makeEdgeGauss(width=info['Blur'],center=(info['lumEdgePos']+info['Gap']))*0.3
    ...
    lum1 = visual.PatchStim(myWin, tex = lumEdge, size = 20.0, units = 'deg', sf=(1/20.0))
    ...
    marker = visual.ShapeStim(myWin, units = 'deg', lineWidth = 1.0, lineColor = 'black', fillColor = None,
                        pos=(markerPos,-5), vertices = ((0, 0.5), (0, -0.5)), closeShape = False)
    marker.draw()
...
#Save stairs
if not os.path.isdir('Noise_%s' %info['participant']):
    os.mkdir('Noise_%s' %info['participant'])
fName = 'Noise_%s//Noise_Gap%.2f_NoiseContrast%.2f_NoiseSize%.2f_Blur%.2f_%s_%s' %(info['participant'], info['Gap'], info['Noise Contrast'], info['Noise Size'], info['Blur'], info['dateStr'], info['participant'])
