...
Conds = ['LMLum']#['LM', 'S', 'Lum', 'LMLum', 'SLum', 'LMS']
shuffle(Conds)
print Conds
...
#Set Up 
    ...
    for thisDistance in stairs:
        ...
        lm= makeRevEdgeGauss(width=info['Blur'],center=(info['lumEdgePos']+info['Gap']))*info['Edge Contrast LM']
...
#                    info['Edge Contrast Lum'], info['dateStr'], thisCond, info['participant'])
    fName = 'MultiLong_%s//SynthLong_Gap%.2f_NContr%.2f_EContr%.2f_Blur%.3f_%s_%s_%s' %(info['participant'], info['Gap'], info['Noise Contrast LM'], \
